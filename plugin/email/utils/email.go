...
//@description: Send email to the specified mailbox due to email middleware error
//@param: subject string, body string
//@return: error

func ErrorToEmail(subject string, body string) error {
	to := strings.Split(global.GlobalConfig.To, ",")
	if to[len(to)-1] == "" { // Determine whether the last element of the slice is empty
		to = to[:len(to)-1]
	}
	return send(to, subject, body)
}
